...
LRESULT CEVFPictureBox::OnEvfDataChanged(WPARAM wParam, LPARAM lParam)
{
	...
	if(pbyteImage != NULL)
	{
		...
		{
			OnDrawFocusRect(pDC, CPoint(data.zoomPosition.x, data.zoomPosition.y), CSize(data.sizeJpegLarge.width, data.sizeJpegLarge.height));
		}
...
void CEVFPictureBox::OnDrawFocusRect(CDC *pDC, CPoint zoomPosition, CSize sizeJpegLarge)
{
	// Get window size.
	CRect rect;
	GetWindowRect(&rect);
	...
	if(m_bDrawZoomFrame)
	{
		...
		CPoint bottomRight(topRight.x, topRight.y + cy / 5);
